...
- ``ARM64ExceptionStepper`` extends ``ARM64Stepper`` for synchronous exception instructions such as ``smc``, ``svc``, ``hvc``, and ``brk``. Instead of relying on a normal next-PC hook, it temporarily routes exceptions through the debugger VBAR so the exception returns control to the debugger after a single step. Targets that cannot service that exception path can opt into ``emulate_exceptions_as_nop`` to step those instructions via a temporary NOP patch.
