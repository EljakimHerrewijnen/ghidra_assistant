...
class Py3BridgeGhidraBackend(GhidraBackend):
    def __init__(self):
    ...
    def get_ghidra_memory_maps_r(self) -> []:
        ...
        Returns:
            [GA_Memory_Segment]
        '''
        memory = self.bridge.remote_eval("[(s.getStart().offset, s.getEnd().offset, s.getSize(), s.comment, s.name, s.getData().readNBytes(s.getSize())) for s in currentProgram.getMemory().blocks]")
        memory_maps = []
        for s in memory:
            start = s[0]
            end = s[1]
            ...
            comment = s[3]
            name = s[4]
            content = self._jarray2bytes(s[5])
            ...
            mem = GA_Memory_Segment(start, end, size, content, name, 0)
