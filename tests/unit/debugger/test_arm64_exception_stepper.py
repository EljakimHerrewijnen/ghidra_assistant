...
from ghidra_assistant.utils.archs.arm64.misc.current_el import CURRENT_EL
...
class _FakeState:
    def __init__(self, current_el: int = 3) -> None:
        self.R_CURRENT_EL = _CurrentEL(current_el)
...
def _make_arch_debugger(*, current_el: int = 3, with_sync_special_regs: bool = True, with_create_debugger_vbar: bool = True):
    shellcode = ShellcodeCrafterARM64(None, None)
    state = _FakeState(current_el)
...
def test_smc_from_el1_swaps_el3_vbar_instead_of_current_el_vbar() -> None:
    arch_dbg = _make_arch_debugger(current_el=1)
    sync_history: list[tuple[int, int, int]] = []

    def sync_special_regs() -> None:
        sync_history.append((arch_dbg.state.VBAR_EL1, arch_dbg.state.VBAR_EL2, arch_dbg.state.VBAR_EL3))
    ...
    assert sync_history == [
        (0x111000, 0x222000, arch_dbg.vector_table_addr),
        (0x111000, 0x222000, original_vbar),
    ]


def test_smc_from_el1_uses_el3_exception_context_names() -> None:
    arch_dbg = _make_arch_debugger(current_el=1)
    device = _FakeConcreteDevice(arch_dbg)
    stepper = ARM64ExceptionStepper(device, 0x1000)
    decoded = next(stepper.sc.cs.disasm(_assemble("smc #0x7777", 0x1000), 0x1000))

    assert stepper.get_exception_target_el(decoded) == 3
    assert stepper.get_active_vbar_name(decoded) == "VBAR_EL3"
    assert stepper.get_active_elr_name(decoded) == "ELR_EL3"


def test_smc_from_el2_uses_el2_exception_context_names() -> None:
    arch_dbg = _make_arch_debugger(current_el=2)
    device = _FakeConcreteDevice(arch_dbg)
    stepper = ARM64ExceptionStepper(device, 0x1000)
    decoded = next(stepper.sc.cs.disasm(_assemble("smc #0x7777", 0x1000), 0x1000))

    assert stepper.get_exception_target_el(decoded) == 2
    assert stepper.get_active_vbar_name(decoded) == "VBAR_EL2"
    assert stepper.get_active_elr_name(decoded) == "ELR_EL2"


@pytest.mark.parametrize(
    ("raw_value", "expected_el"),
    [
        (0x0, 0),
        (0x4, 1),
        (0x8, 2),
...
def test_current_el_decodes_architectural_field(raw_value: int, expected_el: int) -> None:
    assert CURRENT_EL(raw_value).get_exception_level() == expected_el


def test_exception_step_can_emulate_instruction_as_nop() -> None:
    arch_dbg = _make_arch_debugger(current_el=2)
    device = _FakeConcreteDevice(arch_dbg)
    _write_memory(device, 0x1000, _assemble("smc #0x7777", 0x1000))
    _write_memory(device, 0x1004, b"ABCD1234EFGH5678")

    stepper = ARM64ExceptionStepper(device, 0x1000, emulate_exceptions_as_nop=True)
    stepper.step()
    ...
    assert device.read_requests == [4]
    assert stepper.pc == 0x1004
    assert device.memdump_region(0x1000, 4) == _assemble("smc #0x7777", 0x1000)
    assert device.memdump_region(0x1004, 16) == b"ABCD1234EFGH5678"
